...
namespace GraphQL.Server.Test.GraphQL.Models {
	...
	public class PeopleGraphType : ObjectGraphType<People> {
		...
		public PeopleGraphType() {
			...
			this.Field(expression => expression.Films);
			...
			this.Field(expression => expression.Species);
			this.Field(expression => expression.Starships);
			this.Field(expression => expression.Vehicles);
		}
